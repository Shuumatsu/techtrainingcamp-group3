...
func init() {
	...
	if !Client.Migrator().HasTable(&dbmodels.User{}) {
		err = Client.AutoMigrate(&dbmodels.User{})
		if err != nil {
			panic(err)
		}
		Client.Logger = Client.Logger.LogMode(0)
		err = RegisterDefaultUser(config.UserAmount)
		Client.Logger = Client.Logger.LogMode(2)
		if err != nil {
	...
	if !Client.Migrator().HasTable(&dbmodels.Envelope{}) {
		err = Client.AutoMigrate(&dbmodels.Envelope{})
		if err != nil {
			panic(err)
		}
...
func RegisterDefaultUser(n uint64) error {
	...
	doRegister := func(lo, hi uint64) {
		...
		err := Client.Table(dbmodels.User{}.TableName()).Create(users).Error
		if err != nil {
